...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  provisioner "local-exec" {
    command = "echo ${self.public_ip_address} >> ip-publico.txt"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "echo subnet_id = ${data.terraform_remote_state.vnet.outputs.subnet_id} >> subnet-id.txt",
      "echo nsg_id = ${data.terraform_remote_state.vnet.outputs.nsg_id} >> nsg-id.txt",
    ]
  }
  ...
  provisioner "file" {
    ...
    destination = "/tmp/tamanho-vm.txt"
  }
}